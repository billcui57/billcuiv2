...
function ConsoleInput({ setUserInput, userInput,  handlePressedEnter, currDir, handlePressedTab}: {setUserInput: any, handlePressedEnter:any, userInput:any, currDir: string, handlePressedTab:any}) {
  ...
  return (
    <div >
      ...
      <input autoComplete = "off" className="text-console-white text-2xl pt-1 bg-transparent inline outline-none w-10/12 text-pink-500" id = "consoleInput" autoFocus type="text" onChange={handleUserTyping} onKeyDown={handleUserKeyDown} value={userInput}></input>
    </div>
  );
}
